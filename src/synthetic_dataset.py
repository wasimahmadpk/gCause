...
class SyntheticDataset:
    ...
    def __init__(self, root1, root2, time_steps, Tref, C, Tao, dynamic_noise):
        ...
        self.X7 = list(np.zeros(10))
        self.X8 = list(np.zeros(10))
        self.X9 = list(np.zeros(10))
        self.X10 = list(np.zeros(10))
        self.base_1 = list(np.zeros(10))
        self.base_2 = list(np.zeros(10))
    ...
    def generate_data(self):
        ...
        for t in range(10, self.time_steps): 
            ...
            # Root node
            self.base_1.append(self.root1[t] + dynmaic_noise['n1'][t])
            self.base_2.append(self.root2[t] + dynmaic_noise['n2'][t])
            
            # Subsystem: 1
            self.X1.append(C.get('c1') * self.base_1[t - Tao.get('t1')] + dynmaic_noise['n1'][t])
            ...
            self.X4.append(C.get('c4') * self.X1[t - Tao.get('t2')] + C.get('c3') * self.X2[t - Tao.get('t3')] + dynmaic_noise['n3'][t])
            
            
            # Extra
            self.X5.append(C.get('c3') * self.X1[t - Tao.get('t3')]  + C.get('c3') * self.X2[t - Tao.get('t3')]  + C.get('c3') * self.X3[t - Tao.get('t3')] + C.get('c3') * self.X4[t - Tao.get('t3')]  + dynmaic_noise['n4'][t])
            self.X6.append(C.get('c1') * self.X3[t - Tao.get('t2')] + C.get('c3') * self.X5[t - Tao.get('t3')] + dynmaic_noise['n1'][t])
            self.X7.append(C.get('c3') ** self.X4[t - Tao.get('t3')] + dynmaic_noise['n3'][t])
            self.X8.append(C.get('c2') * self.X3[t - Tao.get('t3')] + C.get('c5') * self.X4[t - Tao.get('t3')] + C.get('c3') * self.X5[t - Tao.get('t3')] + dynmaic_noise['n1'][t])
...
if __name__ == '__main__':
    ...
    SAMPLE_RATE = pars["sample_rate"]  # Hertz
    DURATION = pars["duration"]  # Seconds
    ...
    C = {'c1': 0.50, 'c2': 1.50, 'c3': 0.75, 'c4': 1.25, 'c5': 1.60, 'c6': 1.25}           # c2:1.75, c5:1.85
